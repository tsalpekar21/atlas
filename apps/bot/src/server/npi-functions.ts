...
import {
	crawlRagIndexResponseSchema,
	crawlRagQueryRequestSchema,
	crawlRagQueryResponseSchema,
...
export const queryCrawlRag = createServerFn({ method: "POST" })
	.inputValidator((data: unknown) => crawlRagQueryRequestSchema.parse(data))
	.handler(async ({ data }) => {
		const res = await api.npi.rag.query.$post({
			json: {
				query: data.query,
				topK: data.topK ?? 5,
		...
		const json = await jsonWithErrorMessage(res);
		return crawlRagQueryResponseSchema.parse(json);
	});

export const triggerCrawlRagChunk = createServerFn({ method: "POST" })
  .inputValidator((data: { crawlId: string }) => data)
  .handler(async ({ data }) => {
    const res = await api.npi.crawls[":crawlId"].chunk.$post({
      param: { crawlId: data.crawlId },
    });
    if (res.status === 404) {
      throw new Error("Crawl not found");
    ...
    const json = await jsonWithErrorMessage(res);
    return crawlRagIndexResponseSchema.parse(json);
  });
